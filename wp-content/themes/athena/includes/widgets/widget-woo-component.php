...
class Woo_Widget_Component extends WP_Widget {
	...
	public function __construct() {
		...
		$this->woo_widget_componentslist = array(
												'portfolio' => __( 'Portfolio Area', 'woothemes' ), 
												'homepage-intro' => __( 'Homepage Intro', 'woothemes' )
												);

		// Add support for the WooCommerce shelf.
		if ( function_exists( 'is_woocommerce_activated' ) && is_woocommerce_activated() ) {
			$this->woo_widget_componentslist['homepage-shop'] = __( 'Homepage Shop', 'woothemes' );
		}
	...
	public function widget( $args, $instance ) {  
		...
		echo str_replace('<div class="col-full">', '', $before_widget );
		...
		echo preg_replace('/<\/div>/', '', $after_widget, 1);
...
<?php
	...
	protected function load_component ( $component ) {
		switch ( $component ) {
			case 'portfolio':
				get_template_part( 'includes/portfolio-slideshow' );
			break;
			...
			case 'homepage-shop':
				get_template_part( 'includes/home-shop' );
			break;
