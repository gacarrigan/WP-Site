...
class myPinterestBadge {
    ...
    private $cacheFile;
    private $numberOfBoards = 0;
    private $numberOfPins = 0;
    private $numberOfLikes = 0;
    private $numberOfFollower = 0;
    private $following = 0;
    private $pins;
    private $pinboard = '';
    ...
    function __construct($pinid = '', $pinsize = '', $pincounts = '', $devcredit = '', $pinboard = '', $cache_time = '') {
        if (!empty($pinid)) {
            ...
            $this->pinurl = $this->pinterest_url . '/' . trim($pinid) . '/pins/';
            ...
            $this->pinboard_url = $this->pinterest_url . '/' . $pinid . '/' . trim($pinboard) . '/';
            if (!empty($cache_time)) {
    ...
    protected function myPinterestBadge() {
        ...
        if (!empty($this->pinboard)) {
            ...
            $html_pinboard = str_get_html($strdata_from_pinboard);
        }

        $html = str_get_html($strdata);
        if (!empty($this->pinboard)) {
            $items = $html_pinboard->find('a[class=PinImage ImgLink]');
        } else {
            $items = $html->find('a[class=PinImage ImgLink]');
        }
