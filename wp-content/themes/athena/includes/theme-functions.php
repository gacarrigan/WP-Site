	...
	<ul>
		...
		<li class="post-comments">
			<?php if (comments_open()): ?>
				<a href="<?php comments_link(); ?>" title="<?php _e('Post a comment', 'woothemes') ?>"><?php _e('Post a comment', 'woothemes') ?></a>
			<?php endif; ?>
		</li>
		...
		<li class="post-tags">
			<span class="small"><?php _e( 'Tags', 'woothemes' ) ?></span>
			<?php the_tags( '', ', ', '' ); ?>
		</li>
...
?>
	...
	<aside id="connect" class="fix">
		...
		<div <?php if ( $related_posts != '' ) echo 'class="col-left"'; ?>>
			...
			<h4><?php if ( $title ) echo apply_filters( 'widget_title', $title ); else _e('Subscribe','woothemes'); ?></h4>
...
/*-----------------------------------------------------------------------------------*/
	...
	if ( ! function_exists( 'woo_comment_form_fields' ) ) {
		function woo_comment_form_fields ( $fields ) {
			...
			$fields =  array(
				'author' => '<div class="comments-left"><p class="comment-form-author">' .
							'<input id="author" placeholder="' . __( 'Your Name', 'woothemes' ) . ( $req ? $required_text : '' ) . '" class="txt" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' />' .
							'<label for="author">' . __( 'Your Name', 'woothemes' ) . ( $req ? $required_text : '' ) . '</label> ' .
							'</p>',
				'email'  => '<p class="comment-form-email">' .
				            '<input id="email" placeholder="' . __( 'Your Email', 'woothemes' ) . ( $req ? $required_text : '' ) . '" class="txt" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' />' .
				            '<label for="email">' . __( 'Your Email', 'woothemes' ) . ( $req ? $required_text : '' ) . '</label> ' .
				            '</p>',
				'url'    => '<p class="comment-form-url">' .
				            '<input id="url" placeholder="' . __( 'Your Website', 'woothemes' ) . '"  class="txt" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" />' .
				            '<label for="url">' . __( 'Your Website', 'woothemes' ) . '</label>' .
				            '</p></div>',
			);
...
/*-----------------------------------------------------------------------------------*/
	...
	if ( ! function_exists( 'woo_comment_form_settings' ) ) {
		function woo_comment_form_settings ( $settings ) {
			...
			$settings['comment_field'] = str_replace( '<textarea ', '<textarea placeholder="' . esc_attr( __( 'Your Message', 'woothemes'  ) ) . '" ', $settings['comment_field'] );
			$settings['comment_field'] = $settings['comment_field'];
...
function woo_featured_slider_get_slides ( $args ) {
	...
	if ( 0 < intval( $args['term'] ) && isset( $args['taxonomy'] ) ) {
		$query_args['tax_query'] = array(
										array( 'taxonomy' => $args['taxonomy'], 'field' => 'id', 'terms' => intval( $args['term']) )
									);
	}
