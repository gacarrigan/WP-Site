...
class WooDojo_Dynamic_Menu {
	...
	public function meta_box() {		
		...
		foreach ( $post_types as $post_type ) {
			if( post_type_supports( $post_type, 'woodojo-dynamic-menus' ) ) {
				add_meta_box( 'dm_custom_locations', __( 'Custom Menus', 'woodojo-dynamic-menus' ), array( &$this, 'metabox_markup' ), $post_type, 'side', 'default' );
			}
		}
	} // End meta_box()
	 ...
	 * @return void
	 */	
	public function metabox_markup() {
		global $post;
		...
		if ( count( $menu_locations ) == 0 )
			echo '<p>' . __( 'Your theme doesn\'t seem to have any pre-defined menu locations. :(', 'woodojo-dynamic-menus' ) . '</p>';
	} // End metabox_markup()
	 ...
	 * @return void
	 */	
	public function save_custom_menus( $post_id, $post ) {
		if ( !isset( $_POST['dm_noncename'] ) || !wp_verify_nonce( $_POST['dm_noncename'], $this->plugin_basename ))
	    	return $post->ID;
	 ...
	 * @return array $args
	 */	
	public function filter_nav_locations( $args ) {
		global $post;

		if( !post_type_supports( get_post_type( $post->ID ), 'woodojo-dynamic-menus' ) )
			return $args;
	...
	public function add_post_type_support() {
		...
		add_post_type_support( 'page', 'woodojo-dynamic-menus' );
